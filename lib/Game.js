...
module.exports = class Game {
	...
	drawBody(context) {
		...
		if(this.snake.body[0].direction === 'up') {
			this.snake.body[0].drawHeadUpDown(context, this.snake.snakeImages[0]);
		} else if(this.snake.body[0].direction === 'down') {
			this.snake.body[0].drawHeadUpDown(context, this.snake.snakeImages[1]);
		} else if(this.snake.body[0].direction === 'left') {
			this.snake.body[0].drawHeadLeftRight(context, this.snake.snakeImages[2]);
		} else {
			this.snake.body[0].drawHeadLeftRight(context, this.snake.snakeImages[3]);
		}
		...
		if(this.snake.body[this.snake.body.length-1].direction === 'up') {
			this.snake.body[this.snake.body.length-1].drawTailUpDown(context, this.snake.snakeImages[4]);
		} else if(this.snake.body[this.snake.body.length-1].direction === 'down') {
			this.snake.body[this.snake.body.length-1].drawTailUpDown(context, this.snake.snakeImages[5]);
		} else if(this.snake.body[this.snake.body.length-1].direction === 'left') {
			this.snake.body[this.snake.body.length-1].drawTailLeftRight(context, this.snake.snakeImages[6]);
		} else {
			this.snake.body[this.snake.body.length-1].drawTailLeftRight(context, this.snake.snakeImages[7]);
		}
	} 
	...
	raiseScore() {
		...
		if(this.score > this.highScore) {
			...
			document.querySelector('.high-score').innerText = this.highScore;
			document.querySelector('.high-score-message').innerText = "You set a new high score!";
