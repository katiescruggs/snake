const Block = require('./Block.js');

module.exports = class Snake {
  constructor(gameW, gameH) {
    this.x = gameW / 2;
    this.y = gameH / 2;
    this.changeX = -30;
    this.changeY = 0;
    this.gameW = gameW;
    this.gameH = gameH;
    this.headW = 75;
    this.headH = 40;
    this.tailW = 20;
    this.tailH = 62;

    this.snakeImages = [
      document.getElementById('snake-head-up'),     //0
      document.getElementById('snake-head-down'),		//1	
      document.getElementById('snake-head-left'),		//2	
      document.getElementById('snake-head-right'),  //3
      document.getElementById('snake-tail-up'), 		//4
      document.getElementById('snake-tail-down'),		//5	
      document.getElementById('snake-tail-left'),		//6	
      document.getElementById('snake-tail-right'),  //7
      document.getElementById('snake-body-vertical'), //8
      document.getElementById('snake-body-horizontal') //9
    ];

    this.body = [
      new Block (this.x, this.y, 30, 30), 
      new Block(this.x + 30, this.y, 30, 30),
      new Block (this.x + 60, this.y, 30, 30) 
    ];
  }

   drawBody(context) {
    if(this.body[0].direction === 'up') {
      this.body[0].draw(context, this.snakeImages[0]);
    } else if(this.body[0].direction === 'down') {
      this.body[0].draw(context, this.snakeImages[1]);
    } else if(this.body[0].direction === 'left') {
      this.body[0].draw(context, this.snakeImages[2]);
    } else {
      this.body[0].draw(context, this.snakeImages[3]);
    }

    for(var i = 1; i < this.body.length - 1; i++) {
      if(this.body[i].direction === 'left' || this.body[i].direction === 'right') {
        this.body[i].draw(context, this.snakeImages[8]);
      } else {
        this.body[i].draw(context, this.snakeImages[9]);
      }
    }

    if(this.body[this.body.length-1].direction === 'up') {
      this.body[this.body.length-1].draw(context, this.snakeImages[4]);
    } else if(this.body[this.body.length-1].direction === 'down') {
      this.body[this.body.length-1].draw(context, this.snakeImages[5]);
    } else if(this.body[this.body.length-1].direction === 'left') {
      this.body[this.body.length-1].draw(context, this.snakeImages[6]);
    } else {
      this.body[this.body.length-1].draw(context, this.snakeImages[7]);
    }
  } 


  currentDirection() {
    if(this.changeX === -30){
      return 'left';
    } else if(this.changeX === 30) {
      return 'right';
    } else if(this.changeY === -30) {
      return 'up';
    } else if(this.changeY === 30) {
      return 'down';
    }
  }

  move() {
    let direction = this.currentDirection();

	    this.body.pop();
	    this.body.unshift(new Block (this.body[0].x + this.changeX, this.body[0].y + this.changeY, 30, 30, 'green', direction));
  }

  grow() {
    let direction = this.currentDirection();
    this.body.unshift(new Block (this.body[0].x + this.changeX, this.body[0].y + this.changeY, 30, 30, 'green', direction));
  }

  changeDirection(keyCode) {
    let direction = this.currentDirection();

    if(keyCode === 37) {
      if(direction === 'up' || direction === 'down') {
        this.changeX = -30;
        this.changeY = 0;
        this.body[0].direction = 'left';
      }
    } else if(keyCode === 39) {
      if(direction === 'up' || direction === 'down') {
        this.changeX = 30;
        this.changeY = 0;
        this.body[0].direction = 'right';
      }
    } else if(keyCode === 38) {
      if(direction === 'left' || direction === 'right') {
        this.changeX = 0;
        this.changeY = -30;
        this.body[0].direction = 'up';
      }
    } else if(keyCode === 40) {
      if(direction === 'left' || direction === 'right') {
        this.changeX = 0;
        this.changeY = 30;
        this.body[0].direction = 'down';
      }
    }
  }

  onSnakeCoordinates(collider) {
    for (var i = 1; i < this.body.length; i++ ) {
      if (this.body[i].x === collider.x && this.body[i].y === collider.y){
        console.log('HITTING OBJECT');
        return true;
      }
    }
  }

  hitWall() {
    if (this.body[0].x < 0 || this.body[0].x > (this.gameW-30) || this.body[0].y < 0 || this.body[0].y > (this.gameH-30)){

      return true;
    }
  }
};
